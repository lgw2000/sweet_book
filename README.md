...
이 프로젝트의 일반 사용자 인증은 심사용 데모 구조입니다. 로그인 후 프론트엔드가 사용자 ID를 보관하고 API 요청에 함께 보내는 방식이며, 실제 서비스처럼 사용자 세션 토큰이나 JWT로 모든 요청의 권한을 검증하는 구조는 아닙니다.

관리자 기능은 관리자 로그인 시 발급되는 세션 토큰으로 보호합니다. 관리자 계정은 `.env`의 `SWEET_BOOK_ADMIN_ID`, `SWEET_BOOK_ADMIN_PASSWORD`로 설정하며, 일반 회원가입으로 만들 수 없습니다.

업로드 파일은 이미지 파일만 허용합니다.
...
배포용 서비스로 확장하려면 일반 사용자 인증 토큰, CSRF/XSS 방어, 관리자 토큰 만료, 비밀번호 재설정 이메일 흐름을 추가해야 합니다.
